...
@NamedQueries({
        ...
        @NamedQuery(name = Meal.ALL_SORTED_BETWEEN, query = "SELECT m FROM Meal m where m.user.id = :user_id and m.dateTime >= :startDate AND m.dateTime <=  :endDate ORDER BY m.dateTime DESC ")
})
